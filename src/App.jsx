...
import { Plus, Trash2, QrCode, LogOut, Store, Utensils, UtensilsCrossed, Check, X, ArrowLeft, ArrowRight, Copy, Eye, EyeOff, ChefHat, Camera, Loader2, ImagePlus, RefreshCw, Printer, Languages, Filter, Lock } from "lucide-react";
...
async function extraerCartaDeFoto(base64, mediaType) {
  ...
  return parsed.categorias.map((cat) => ({
    ...
    items: (cat.items || []).map((it) => ({
      ...
      alergenos: Array.isArray(it.alergenos) ? it.alergenos.filter((a) => ALERGENOS.includes(a)) : [],
...
async function generarDemoRestaurantes() {
  ...
  const it = (nombre, precio, desc, alergenos = [], disponible = true, variantes = [], grupos = []) => ({
    ...
    grupos: grupos.map((g) => ({
      id: uid(),
      titulo: g.titulo,
      opciones: g.opciones.map((o) => ({ id: uid(), nombre: o.nombre, precio: o.precio || "", alergenos: o.alergenos || [] })),
    })),
  });
  ...
  return (
    <div style={S.center}>
      <div style={{ textAlign: "center", marginBottom: 36 }}>
        ...
        <p style={S.sub}>Cartas digitales para restaurantes</p>
      </div>

      <div style={S.cardPrincipalAcceso}>
        <div style={S.iconoPrincipalAcceso}>
          <Utensils size={22} color="#C9A227" />
        </div>
        <h2 style={S.tituloPrincipalAcceso}>Panel del restaurante</h2>
        <p style={S.subPrincipalAcceso}>
          Accede para gestionar tu carta, precios y disponibilidad en cualquier momento.
        </p>
        <button style={S.btnEntrarPanel} onClick={onCliente}>
          Entrar a mi panel <ArrowRight size={16} />
        </button>
      </div>

      <div style={S.ctaNuevoCliente}>
        <p style={S.ctaNuevoClienteTexto}>¿Tienes un restaurante y quieres una carta digital como esta?</p>
        <a
          href="https://wa.me/34654030531?text=Hola%2C%20quiero%20una%20carta%20digital%20para%20mi%20restaurante"
          target="_blank"
          rel="noopener noreferrer"
          style={S.btnWhatsappOutline}
        >
...
function ClientePanel({ restaurante, restaurantes, setRestaurantes, onSalir, onVerPublica, onVerCartel }) {
  ...
  // Analiza TODOS los platos de la carta activa de una sola vez (no hace falta ir
  // plato por plato). Nunca decide de forma definitiva: si no está segura de un
  // plato, lo deja marcado con "necesitaRevision" para que el dueño lo compruebe.
  const [analizandoAlergenosCarta, setAnalizandoAlergenosCarta] = useState(false);
  const sugerirAlergenosCartaCompleta = async () => {
    const todosLosItems = cartaActiva.categorias.flatMap((cat) => cat.items);
    if (todosLosItems.length === 0) {
      alert("Esta carta todavía no tiene platos que analizar.");
      return;
    }
    setAnalizandoAlergenosCarta(true);
    try {
      const response = await fetch("/api/sugerir-alergenos-carta", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
          ...
          c.id === cartaActiva.id
            ? {
                ...
                categorias: c.categorias.map((cat) => ({
                  ...cat,
                  items: cat.items.map((it) => {
                    const res = resultadosPorId.get(it.id);
                    if (!res) return it;
                    const alergenosBase = Array.isArray(res.alergenosBase)
                      ? res.alergenosBase.filter((a) => ALERGENOS.includes(a))
                      : [];
                    const opcionesConAlergenos = Array.isArray(res.opcionesConAlergenos)
                      ? res.opcionesConAlergenos
                      : [];
                    return {
                      ...it,
                      alergenos: alergenosBase,
                      necesitaRevision: !!res.necesitaRevision,
                      grupos: (it.grupos || []).map((g) => ({
                        ...g,
                        opciones: g.opciones.map((o) => {
                          const encontrada = opcionesConAlergenos.find((oc) => oc.id === o.id);
                          return {
                            ...o,
                            alergenos: encontrada
                              ? (encontrada.alergenos || []).filter(
                                  (a) => ALERGENOS.includes(a) && !alergenosBase.includes(a)
                                )
                              : [],
                          };
                        }),
      ...
      alert("No se ha podido analizar la carta ahora mismo. Puedes marcar los alérgenos a mano.");
    } finally {
      setAnalizandoAlergenosCarta(false);
    }
  };
  ...
  const addGrupoOpciones = (catId, itemId) => {
    modificarItem(catId, itemId, (i) => ({
      ...
      grupos: [
        ...(i.grupos || []),
        { id: uid(), titulo: "Nuevas opciones", opciones: [{ id: uid(), nombre: "", precio: "", alergenos: [] }] },
      ],
    }));
  };
  ...
  const addOpcionAGrupo = (catId, itemId, grupoId) => {
    modificarItem(catId, itemId, (i) => ({
      ...
      grupos: (i.grupos || []).map((g) =>
        g.id === grupoId
          ? { ...g, opciones: [...g.opciones, { id: uid(), nombre: "", precio: "", alergenos: [] }] }
          : g
      ),
    }));
  };
  ...
  // Ordena las opciones de un grupo: primero las que no llevan suplemento (precio en blanco),
  // luego el resto de menor a mayor precio extra. Mismo criterio que con tapa/media/ración.
  const ordenarOpcionesPorPrecio = (catId, itemId, grupoId) => {
    modificarItem(catId, itemId, (i) => ({
      ...i,
      grupos: (i.grupos || []).map((g) =>
        g.id === grupoId
          ? {
              ...g,
              opciones: [...g.opciones].sort((a, b) => {
                const pa = a.precio ? parseFloat(a.precio) || 0 : -1;
                const pb = b.precio ? parseFloat(b.precio) || 0 : -1;
                return pa - pb;
              }),
            }
          : g
  ...
  return (
    <div style={S.page}>
      ...
      {/* Contador de visitas */}
      <div style={S.visitasCard}>
        <Eye size={16} color="#C9A227" />
        <span>
          Tu carta se ha visto <b>{restaurante.visitas || 0}</b> {restaurante.visitas === 1 ? "vez" : "veces"}
        </span>
      </div>
      ...
      <div style={{ marginTop: 18 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            <button
              style={S.btnGhostSmall}
              onClick={sugerirAlergenosCartaCompleta}
              disabled={analizandoAlergenosCarta}
            >
              {analizandoAlergenosCarta ? <Loader2 size={13} className="animate-spin" /> : <Filter size={13} />}
              {analizandoAlergenosCarta ? "Analizando toda la carta…" : "Sugerir alérgenos (IA) — toda la carta"}
            </button>
            <input
        ...
        {analizandoAlergenosCarta && (
          <div style={{ ...S.muted, marginBottom: 10 }}>
            Analizando los alérgenos de todos los platos de esta carta… puede tardar unos segundos si hay muchos.
          </div>
        )}
        <input
              ...
              <div key={it.id} style={S.itemCard}>
                      ...
                      <div key={o.id} style={{ marginBottom: 6 }}>
                        <div style={{ display: "flex", gap: 6 }}>
                          ...
                          <input
                            ...
                            onBlur={(e) => {
                              updateOpcion(cat.id, it.id, g.id, o.id, "precio", corregirPrecio(e.target.value));
                              ordenarOpcionesPorPrecio(cat.id, it.id, g.id);
                            }}
                        ...
                        {o.alergenos && o.alergenos.length > 0 && (
                          <div style={S.opcionAlergenosTag}>
                            Solo si eliges esto: {o.alergenos.join(", ")}
                          </div>
                        )}
                      </div>
                    ))}
                ...
                {it.necesitaRevision && (
                  <div style={{ marginTop: 10 }}>
                    <span style={S.avisoRevision}>⚠ Revisar alérgenos</span>
                  </div>
                )}
...
function CartaPublica({ restaurantes, slugInicial, onVolver }) {
  ...
  // Cuenta una visita real (una única vez por carga) para que el dueño vea cuánta
  // gente mira su carta. No se cuenta si está bloqueada por impago.
  const visitaRegistradaRef = useRef(false);
  useEffect(() => {
    if (restaurante && restaurante.pago !== "pendiente" && !visitaRegistradaRef.current) {
      visitaRegistradaRef.current = true;
      fetch("/api/registrar-visita", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ slug: restaurante.slug }),
      }).catch(() => {}); // si falla, no molestamos al comensal con nada
    }
  }, [restaurante]);
  ...
  return (
    <div style={S.publicWrap}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {(it.grupos || []).map((g) => (
                    <div key={g.id} style={S.publicGrupoOpciones}>
                      {g.titulo && g.titulo.trim() && (
                        <div style={S.publicGrupoTituloBloque}>{g.titulo}</div>
                      )}
                      {[...g.opciones]
                        .sort((a, b) => {
                          const pa = a.precio ? parseFloat(a.precio) || 0 : -1;
                        ...
                        .map((o) => (
                          <div key={o.id} style={S.publicOpcionBloque}>
                            <div style={S.publicOpcionFila}>
                              <span>{o.nombre}</span>
                              <span style={S.publicOpcionDerecha}>{o.precio ? `+${o.precio} €` : ""}</span>
                            </div>
                            {o.alergenos && o.alergenos.length > 0 && (
                              <div style={S.opcionAlergenoPublico}>
                                {o.alergenos.map((a) => traducirAlergeno(a, idioma)).join(", ")}
                              </div>
                            )}
                          </div>
                        ))}
      ...
      <a
        href="https://wa.me/34654030531?text=Hola%2C%20he%20visto%20una%20carta%20digital%20hecha%20con%20Mejores%20Cartas%20y%20me%20interesa%20para%20mi%20restaurante"
        target="_blank"
        rel="noopener noreferrer"
        style={S.publicMarcaPropia}
      >
        Carta digital creada con Mejores Cartas
      </a>
    </div>
  );
}
...
const S = {
  ...
  cardPrincipalAcceso: {
    background: "linear-gradient(180deg, #211F1B 0%, #1C1A16 100%)",
  ...
  ctaNuevoCliente: {
    ...
    marginTop: 34,
    textAlign: "center",
    borderTop: "1px solid #2A281F",
    paddingTop: 28,
  },
  ctaNuevoClienteTexto: {
    color: "#7A7468",
    fontSize: 12.5,
    marginBottom: 14,
    lineHeight: 1.5,
  ...
  publicGrupoOpciones: { fontSize: 11.5, color: "#9B9384", marginTop: 8, lineHeight: 1.5 },
  publicGrupoTituloBloque: {
    color: "#C9A227",
    fontWeight: 600,
    fontSize: 11.5,
    marginBottom: 3,
