...
import { Plus, Trash2, QrCode, LogOut, Store, Utensils, Check, X, ArrowLeft, Copy, Eye, EyeOff, ChefHat, Camera, Loader2, ImagePlus, RefreshCw, Printer, Languages, Filter, Lock } from "lucide-react";
...
// Contraseña del panel superadmin (tú). Cámbiala aquí antes de usarlo en producción.
...
// Guardamos todo el array de restaurantes en una única fila de la tabla app_data,
// bajo la clave "restaurantes". Es el equivalente en base de datos real
// a lo que hacíamos con window.storage durante las pruebas.
...
async function saveRestaurantes(list) {
  try {
    const { error } = await supabase
      .from("app_data")
      .upsert({ key: "restaurantes", value: list });
...
export default function App() {
  ...
  const [vista, setVista] = useState("inicio"); // inicio | admin-login | super | cliente-login | cliente-panel | publica | cartel
  const [clienteActivo, setClienteActivo] = useState(null); // id restaurante logueado
  const [slugPublico, setSlugPublico] = useState(null);
  const [slugCartel, setSlugCartel] = useState(null);
  const [sesionAdmin, setSesionAdmin] = useState(false);

  useEffect(() => {
    loadRestaurantes().then((r) => setRestaurantes(r));
  }, []);

  const persist = useCallback(async (next) => {
    setRestaurantes(next);
    await saveRestaurantes(next);
  }, []);
  ...
  return (
    <div style={S.app}>
      ...
      {vista === "cartel" && (
        <CartelQR restaurantes={restaurantes} slug={slugCartel} onVolver={() => setVista("inicio")} />
      )}
    </div>
  );
}
  ...
  return (
    <div style={S.center}>
      <div style={{ textAlign: "center", marginBottom: 36 }}>
        <div style={S.logoMark}>QR</div>
        <h1 style={S.h1}>Cartas al instante</h1>
  ...
  return (
    <div style={S.center}>
      ...
      <div style={S.card}>
        ...
        <button style={{ ...S.btnPrimary, width: "100%", marginTop: 14 }} onClick={entrar}>
          Entrar
        </button>
      </div>
    </div>
            ...
            <div key={r.id} style={S.rowCard}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, flex: 1 }}>
                ...
                <div>
                  ...
                  <div style={S.rowSub}>/{r.slug} · usuario panel: {r.slug}</div>
                </div>
              </div>
              <span
                style={{
...
function ClienteLogin({ restaurantes, onEntrar, onVolver }) {
  ...
  const entrar = async () => {
    const r = restaurantes.find((x) => x.slug === slug.trim());
...
function ClientePanel({ restaurante, restaurantes, setRestaurantes, onSalir, onVerPublica, onVerCartel }) {
  ...
  const eliminarPlato = (catId, itemId) => {
    update((r) => {
      r.cartas = r.cartas.map((c) =>
        c.id === cartaActiva.id
          ? {
            ...
            {cat.items.map((it) => (
              <div key={it.id} style={S.itemCard}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 10 }}>
                  ...
                  <button style={S.iconBtnDanger} onClick={() => eliminarPlato(cat.id, it.id)}>
                    <Trash2 size={14} />
                  </button>
                </div>
              </div>
  ...
  return (
    <div style={S.publicWrap}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
        <button style={S.publicBack} onClick={onVolver}>
...
function CartelQR({ restaurantes, slug, onVolver }) {
  ...
  // Nota: mientras no haya dominio propio, el QR apunta a una URL de ejemplo con el slug real,
  // para que en cuanto se despliegue en un dominio de verdad solo haya que reimprimir con la URL final.
  const urlEjemplo = `https://tuweb-madre.com/${restaurante.slug}`;
    ...
    <div style={S.page}>
      ...
      <div className="no-print">
        ...
        <div style={{ ...S.muted, marginBottom: 16 }}>
          Esta URL (<b>{urlEjemplo}</b>) es de ejemplo — cuando tengas dominio real, se regenera el cartel con la
          URL final antes de imprimir.
        </div>
...
const S = {
  ...
  h1: { fontFamily: F.display, fontSize: 28, margin: 0, letterSpacing: -0.5 },
  ...
  rowSub: { fontSize: 11, color: "#8A8478", marginTop: 2 },
  badge: {
    fontSize: 10,
    padding: "4px 9px",
    borderRadius: 20,
