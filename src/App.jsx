...
function ClientePanel({ restaurante, restaurantes, setRestaurantes, onSalir, onVerPublica, onVerCartel }) {
  ...
  // Pide a la IA que sugiera los alérgenos de un plato según su nombre/descripción.
  // Nunca decide sola de forma definitiva: si no está segura, deja los alérgenos
  // vacíos y marca "necesitaRevision" para que el dueño lo compruebe a mano.
  const [sugiriendoAlergenos, setSugiriendoAlergenos] = useState(null); // itemId en curso
  const sugerirAlergenosIA = async (catId, itemId, nombre, desc, grupos) => {
    if (!nombre || !nombre.trim()) {
      alert("Ponle primero un nombre al plato para poder sugerir alérgenos.");
      return;
    }
    setSugiriendoAlergenos(itemId);
    try {
      const response = await fetch("/api/sugerir-alergenos", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ nombre, desc, grupos }),
      });
      const data = await response.json();
      if (data.error) throw new Error(data.error);
      const textBlock = (data.content || []).find((b) => b.type === "text");
      ...
      const clean = textBlock.text.replace(/```json|```/g, "").trim();
      const parsed = JSON.parse(clean);
      const alergenosBase = Array.isArray(parsed.alergenosBase)
        ? parsed.alergenosBase.filter((a) => ALERGENOS.includes(a))
        : [];
      const opcionesConAlergenos = Array.isArray(parsed.opcionesConAlergenos) ? parsed.opcionesConAlergenos : [];

      modificarItem(catId, itemId, (i) => ({
        ...i,
        alergenos: alergenosBase,
        necesitaRevision: !!parsed.necesitaRevision,
        // A cada opción añadible (tipo de pan, ingredientes...) le asignamos SOLO
        // sus propios alérgenos, sin mezclarlos con los del plato base — el cliente
        // puede elegir no añadir esa opción, así que no debe figurar como si fuera fija.
        grupos: (i.grupos || []).map((g) => ({
          ...g,
          opciones: g.opciones.map((o) => {
            const encontrada = opcionesConAlergenos.find(
              (oc) => oc.nombre && oc.nombre.trim().toLowerCase() === o.nombre.trim().toLowerCase()
            );
            const alergenosOpcion = encontrada
              ? (encontrada.alergenos || []).filter((a) => ALERGENOS.includes(a))
              : [];
            return { ...o, alergenos: alergenosOpcion };
          }),
        })),
      }));
    } catch (e) {
      alert("No se ha podido sugerir alérgenos ahora mismo. Puedes marcarlos a mano.");
    } finally {
      setSugiriendoAlergenos(null);
    }
  };
            ...
            {cat.items.map((it) => (
              <div key={it.id} style={S.itemCard}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 10, flexWrap: "wrap" }}>
                  <button
                    style={S.btnGhostSmall}
                    onClick={() => sugerirAlergenosIA(cat.id, it.id, it.nombre, it.desc, it.grupos)}
                    disabled={sugiriendoAlergenos === it.id}
                  >
                    {sugiriendoAlergenos === it.id ? (
                      <Loader2 size={12} className="animate-spin" />
                    ) : (
                      <Filter size={12} />
                    )}
                    {sugiriendoAlergenos === it.id ? "Analizando…" : "Sugerir alérgenos (IA)"}
                  </button>
                  {it.necesitaRevision && <span style={S.avisoRevision}>⚠ Revisar alérgenos</span>}
                </div>
