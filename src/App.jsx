...
// Antes se guardaba todo en un único bloque, así que editar cualquier restaurante
// reescribía la base de datos entera — si dos guardados se solapaban, uno podía
// perder los cambios del otro (esto causó el bug del estado de pago). Ahora, editar
// un restaurante solo toca su propia fila; los demás nunca se ven afectados.
const claveDe = (slug) => `restaurante:${slug}`;

async function guardarRestauranteIndividual(r) {
  const { error } = await supabase.from("app_data").upsert({ key: claveDe(r.slug), value: r });
  if (error) throw error;
}

async function borrarRestauranteIndividual(r) {
  const { error } = await supabase.from("app_data").delete().eq("key", claveDe(r.slug));
  if (error) throw error;
}
...
async function loadRestaurantes() {
  ...
  if (error) throw error;
  let lista = (data || []).map((row) => row.value);

  // Migración automática (una sola vez): si todavía no hay filas individuales pero
  // existe el bloque antiguo de antes de este arreglo, lo repartimos en filas nuevas.
  if (lista.length === 0) {
    const { data: viejo } = await supabase.from("app_data").select("value").eq("key", "restaurantes").maybeSingle();
    if (viejo && Array.isArray(viejo.value) && viejo.value.length > 0) {
      lista = viejo.value;
      await Promise.all(lista.map((r) => guardarRestauranteIndividual(r).catch(() => {})));
    }
  }
  return lista;
}
...
export default function App() {
  ...
  // (no reescribe toda la base de datos), evitando que un guardado pise a otro.
  const persist = useCallback(async (next) => {
    ...
    try {
      await Promise.all([
        ...cambiados.map((r) => guardarRestauranteIndividual(r)),
        ...eliminados.map((r) => borrarRestauranteIndividual(r)),
      ]);
    } catch (e) {
      console.error("Error guardando en Supabase:", e);
      alert(
        "⚠ El cambio no se ha podido guardar en la base de datos. Revisa tu conexión y vuelve a intentarlo.\n\nDetalle técnico: " +
          (e?.message || String(e))
      );
    }
  }, []);
                        ...
                        {it.variantes.map((v) => (
                          <div key={v.id} style={{ display: "flex", gap: 6, marginBottom: 6 }}>
                            ...
                            <input
                              ...
                              onBlur={() => ordenarVariantesPorPrecio(cat.id, it.id)}
                              placeholder="0.00"
                            />
                            <button
